...
| 两者都没有 | 优先复用同路径译文的唯一 `link`，否则使用去掉 locale 的文件路径 | 保持语言配对与旧路径语义 |
| 同语言链接重复，或对应译文给出多个候选 `link` | 报错并阻止整批写入 | 不猜测、不产生半套 URL |
...
`link`。如果英文文章机械地使用自身路径，它会与默认语言失去同一公开 slug。迁移器会先收集对应文章已有的稳定链接；只有候选
唯一时才自动复用，候选互相矛盾时宁可让用户决定。
...
- 使用 `node:22-alpine`、pnpm 9.15.1 和 frozen lockfile 的生产 Docker 镜像构建成功。
